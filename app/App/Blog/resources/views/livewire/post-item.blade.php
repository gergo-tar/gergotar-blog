<article class="py-6 border-b border-grey-lighter ">
    ...
    <span
        class="inline-block px-2 py-1 mb-4 text-sm rounded-full bg-green-light font-body text-green">{{ $post->category->translation->name }}</span>
    {{-- Featured image --}}
    ...
    {{-- Title --}}
    <a href="{{ $post->translation->url }}"
        class="block mt-4 text-lg font-semibold transition-colors font-body text-primary hover:text-green dark:text-white dark:hover:text-secondary"
        wire:navigate>
        {{ $post->translation->title }}
    </a>
    {{-- Excerpt --}}
    <p class="text-gray-800 dark:text-grey-lighter">
        <a href="{{ $post->translation->url }}" wire:navigate>
    ...
    <div class="flex flex-col pt-4 sm:flex-row sm:items-center">
        ...
        <p class="mt-4 font-light font-body text-primary dark:text-white sm:mt-0">
            <span class="font-body text-grey dark:text-white sm:pl-2">//</span>
