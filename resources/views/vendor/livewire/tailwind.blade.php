                ...
                <div class="flex pt-8 lg:pt-16">
                    <span class="relative z-0 inline-flex rounded-md shadow-sm rtl:flex-row-reverse">
                        ...
                        @foreach ($elements as $element)
                            ...
                            @if (is_array($element))
                                @foreach ($element as $page => $url)
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}" class="py-1">
                                        @if ($page == $paginator->currentPage())
                                            <span aria-current="page">
                                                <span class="px-3 py-1 font-medium border-2 cursor-pointer border-secondary font-body text-secondary{{ ! $paginator->onFirstPage() ? ' ml-3' : '' }}">{{ $page }}</span>
                                            </span>
                                        @else
                        ...
                        <div>
                            {{-- Next Page Link --}}
                            @if ($paginator->hasMorePages())
                                <button wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" class="flex items-center px-3 py-1 ml-3 font-medium transition-colors border-2 cursor-pointer group border-primary font-body text-primary hover:border-secondary hover:text-secondary dark:border-green-light dark:text-white dark:hover:border-secondary dark:hover:text-secondary" aria-label="{{ __('pagination.next') }}">
                                    {!! __('pagination.next') !!}
